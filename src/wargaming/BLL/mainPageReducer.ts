...
let mainPageReducer = (state = initState, action: any) => {
    switch (action.type) {
...
export const setSelectElementsGlobalState = (selectElements: any) => ({type: SET_SELECT_ELEMENTS, selectElements})
export const setUnSelectElement = (id: any) => ({type: SET_UN_SELECT_ELEMENT, id})
export const setSearch = (value: any) => ({type: SET_SEARCH, value})
export const setFilter = (value: any) => ({type: SET_FILTER, value})


export const openCloseDialogSelector = () => (dispatch: any) => {
    dispatch(showDialogSelector())
    dispatch(selectButtonEnabled())
}

export const setSelectElements = (selectElements: any) => (dispatch: any) => {
